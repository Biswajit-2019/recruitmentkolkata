...
class Hindi_admin_model extends CI_Model {
	...
	function delete($id)
	{
		$query=$this->db->delete('hindi_cms_pages', array('id' => $id)); 
		if($query==true)
		{
			return 1;
		}
	...
	{
		$query=$this->db->delete('hindi_banner', array('id' => $id)); 
		if($query==true)
		{
			return 1;
		}
	...
	{
		$query = $this->db->get_where('hindi_cms_pages', array('id' => $id));
	...
	{
		$query = $this->db->get_where('hindi_banner', array('id' => $id));
        return $query->result();
	}
	...
	function updateData($row,$table,$where_clause)
	{
	...
	function updateData1($table,$data)
	{
		$id=$data['id'];

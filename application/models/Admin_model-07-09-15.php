...
class Admin_model extends CI_Model {
	...
	function getCounts($sql)
	{
		...
		return $query->result();
	}	function delete($id)	{				$query=$this->db->delete('cms_pages', array('id' => $id)); 		if($query==true)		{			return 1;		}	}	function view_edit_cms($id)	{		$query = $this->db->get_where('cms_pages', array('id' => $id));		//$query = $this->db->query("select * from cms_pages where id='$id'");        return $query->result();				}	function updateData($table,$data)	{				$id=$data['id'];		$this->db->where('id', $id);		$this->db->update($table,$data);			}

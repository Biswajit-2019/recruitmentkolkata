...
public function __construct()  
	...
	public function index()
	{
		if($this->session->userdata('user_id')){
			...
			$statement = "`advertise_mst`";
			$data['cms'] =$this->Admin_model->getAll('advertise_mst',$param=array('status'=>'Y'),$order_by_fld='',$order_by='',$limit,$startpoint);
			...
			$this->load->view('admin/recruitment/advertisement',$data);
		...
		if($this->session->userdata('user_id')){
			if($this->input->post('advtNo')){
				...
				$data = array('guideline_status'=>$this->input->post('guideline_status'),'advt_no'=>$this->input->post('advtNo'),'date'=>$date,'vacancy_description'=>$this->input->post('vacancyDescription'),'vacancy_details'=>$vacancyDetail,'guideline'=>$guidelines,'important_content'=>$this->input->post('important'),'status'=>'Y','create_date'=>date("Y-m-d H:i:s"),'update_date'=>date("Y-m-d H:i:s"));
						$create = $this->Common_model->insertValue('advertise_mst', $data);
						if ($create > 0) {
				...
				$this->load->view('admin/includes/header');
				$this->load->view('admin/recruitment/add-advertisement');
				$this->load->view('admin/includes/footer');
			}
	...
	public function edit()
	{
		if($this->session->userdata('user_id')){
			if($this->input->get('id')){
				...
				$this->load->view('admin/includes/header');
				$this->load->view('admin/recruitment/update-advertisement',$data);
				$this->load->view('admin/includes/footer');
	...
	public function uploade()
	{
		...
		if (isset($_FILES['file']) && !empty($_FILES['file']['name'])) {
			...
			if($this->upload->do_upload('file')){
				...
				$temp = $this->Admin_model->updateValue(array('result_status'=>$this->input->post('result_status'),'result'=>$resultDetail,'job_status'=>'N'),'advertise_mst',array('id'=>$this->input->post('id')));
				$this->session->set_flashdata('success', 'Updated Successfully');
				redirect('admin/advertisement/');
			}else{
				$this->session->set_flashdata('errors', $this->upload->display_errors());
	...
	public function result()
	{
		if($this->session->userdata('user_id')){
			...
			$this->load->view('admin/includes/header');
			$this->load->view('admin/recruitment/publish-result',$data);
			$this->load->view('admin/includes/footer');
		}else{
			redirect('admin');
		}
	...
	public function delete()
	{
		if($this->session->userdata('user_id')){
			if($this->input->get('id')){
				$this->Admin_model->updateValue(array('status'=>'N'),'advertise_mst',array('id'=>$this->input->get('id')));
